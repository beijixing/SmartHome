...
class Route
{
    ...
    // REST路由操作方法定义
    private static $rest = [
        'index_controller'  => ['get', '', 'index_controller'],
        ...
        if (!empty($rules)) {
            ...
            if (!empty($item)) {
                if (isset($item['[bind]'])) {
                    ...
                    if (0 === strpos($result, '\\')) {
                        // 绑定到命名空间 例如 \app\index_controller\behavior
                        self::$bind = ['type' => 'namespace', 'namespace' => $result];
                    } elseif (0 === strpos($result, '@')) {
                        // 绑定到类 例如 @app\index_controller\controller\User
                        self::$bind = ['type' => 'class', 'class' => substr($result, 1)];
                    } else {
                        // 绑定到模块/控制器 例如 index_controller/user
     ...
     * @access public
     * @param string    $url URL地址
     * @param string    $controller 控制器名 （支持带模块名 index_controller/user ）
     * @param string    $depr URL分隔符
     * @return array
     */
